...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 List<Integer> twoHopNodes = new ArrayList<>();
		 
		 List<Integer> neighbors = adjListsMap.get(v);
		 if (neighbors != null) {
			 for (Integer neighbor : neighbors) {
				 List<Integer> twoHopNeighbors = adjListsMap.get(neighbor);
				 if (twoHopNeighbors != null) {
					 for(Integer twoHopNeighbor : twoHopNeighbors) {
						 if (twoHopNeighbor != null) {
							 twoHopNodes.add(twoHopNeighbor);
						 }						 
					 }
				 }				 								
			 }
